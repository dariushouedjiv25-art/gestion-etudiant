...
void ajouter_etudiant() {
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; j < nbnote; j++) {
      ...
      } while ((ESTNOMBRE(saisie) == 1) ||
        (etudiants[nb_etudiant].notes[j] < 0) ||
        (etudiants[nb_etudiant].notes[j] > 20));
    }
    etudiants[nb_etudiant].moyenne =
        CalculMoyenne(etudiants[nb_etudiant].notes);
    nb_etudiant++;
  }
...
void trie_tableau() {
  ...
  printf("etudiants trier par moyenne avec succes\n");
}
